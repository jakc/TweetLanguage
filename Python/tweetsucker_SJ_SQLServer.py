...
## ToDo:
##  - WTF is going on with CLD.  Its fine with most languages, but not confident
## on my unicode/UTF8 translation before passing to CLD.
## See comments on
## http://blog.mikemccandless.com/2011/10/language-detection-with-googles-compact.html
##  - Look into compiling the regex into an object
##  - Avoid repeat connecting to the DB
...
def parse_tweet(in_t):
    ...
    try:
        cur.execute("INSERT INTO tweets (geom, text, x, y, swearing, language, date, userid) VALUES (%s,'%s', '%s', '%s', '%i', '%s', '%s', '%s')" % (geom_type, clean, x, y, int(swearing), lan, d, user_id_str))
    except TypeError:
